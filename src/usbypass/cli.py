...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="usbypass",
        description="Use a physical USB drive as a password-bypass key for Linux PAM.",
    )
...
def _cmd_enroll(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 1
    print("USB key enrolled:")
    for k, v in result.items():
        print(f"  {k}: {v}")
    print("\nNext steps:")
    print(f"  1. Unplug and re-plug the USB.")
    print(f"  2. Run `usbypass status` — it should show the key as verified.")
    print(f"  3. Open a new shell and try `sudo -k; sudo echo ok` — no password expected.")
    return 0
...
def _cmd_list(_: argparse.Namespace) -> int:
    ...
    if not registry:
        print("No USB keys enrolled.")
        return 0
    for user, entries in sorted(registry.items()):
        print(f"{user}:")
        for e in entries:
            print(f"  - serial={e.get('serial')}  label={e.get('label')}  enrolled_at={e.get('enrolled_at')}")
    return 0
...
def _cmd_status(_: argparse.Namespace) -> int:
    from usbypass import enrollment, state, usb

    st = state.read_state()
    print("Verified-state file:", state.state_file_path())
    if st is None:
        print("  (no active USB key)")
    else:
        print(f"  user     : {st.get('username')}")
        print(f"  serial   : {st.get('serial')}")
        print(f"  devnode  : {st.get('devnode')}")
        print(f"  verified : {st.get('verified_at')}")
    ...
    if not registry:
        ...
        for user, entries in sorted(registry.items()):
            for e in entries:
                print(f"  {user}: {e.get('label')} [{e.get('serial')}]")

    try:
        parts = usb.list_usb_partitions()
    except Exception as exc:  # pyudev not available
        print(f"\nUSB enumeration failed: {exc}")
        return 0
    ...
    if not parts:
        ...
        for p in parts:
            ...
            tag = "[enrolled]" if enrolled else ""
            print(f"  {p.devnode}  serial={p.serial}  mount={p.mountpoint}  {tag}")
    return 0


def _cmd_doctor(_: argparse.Namespace) -> int:
    from usbypass import config, crypto
    ...
    print("USBYPASS doctor:")
    check(
        f"Secret present at {config.SECRET_PATH}",
        config.SECRET_PATH.exists(),
        "run install.sh or `usbypass install`",
    )
    try:
        crypto.load_secret()
    ...
    check(f"PAM helper present at {helper}", helper.exists())

    udev_rule = Path("/etc/udev/rules.d/99-usbypass.rules")
    check(f"udev rule present at {udev_rule}", udev_rule.exists())

    cli = Path("/usr/local/bin/usbypass")
    check(f"CLI shim present at {cli}", cli.exists() or cli.is_symlink())

    print(f"\nDistro family: {detect_family()}")
